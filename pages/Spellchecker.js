...
export default function Chat() {
  ...
  const sendChat = async (message) => {
    const { data } = await axios.post("/api/chatcoorect", {
      prompt: message,
      sessionId: sessionId,
    });
  ...
  return (
    <div className="container h-screen flex flex-col bg-gray-100 text-gray-900 px-6 py-4 mx-auto">
      <h1 className="text-4xl font-bold mb-4 text-center">المصحح الاملائي</h1>
      ...
      >
        <input
          ...
          placeholder="اكتب  الكلمة او العبارة لتصحيحها..."
          style={{direction: "rtl"}}
        />
        <button
          type="submit"
